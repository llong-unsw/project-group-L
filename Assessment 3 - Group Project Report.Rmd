...
subtitle: |
  | Team L 
  | Hex 5, 2023 ZZSC9020
  | Github link: https://github.com/llong-unsw/project-group-L
author: 
  - "Louis Long (z5162359)"
...
# 2. ensure the relevant packages in this project has been installed
# this can be done by opening Code/01 Setup.R and running the script in RStudio

# 3. open Code/03 Modelling.R and check the parameters
...
Short term electricity demand forecasting plays a key role in helping electricity generators plan and optimise their generation schedules. With the increasing uptake in renewable electricity generation, this places even more importance on having a more accurate load forecasting for especially for quick start electricity generators who help to stabilise the grid when there is limited solar and wind electricity. A more accurate forecast helps each generator to make more informed economic decisions improving profitability. This paper sets out to improve upon the Australian Energy Market Operator's (AEMO) explanatory short term electricity demand forecasting model by focusing on the predictive capability of supplementary variables and investigating more sophisticated machine learning techniques. Each of our models was able to outperform the AEMO benchmark forecast. We identify that the inclusion of lagged demand (akin to autoregression) helps to simplify our models and limits the number of additional explanatory variables needed to forecast short term electricity demand. In addition, we find that support vector regression's ability to model the nonlinear relationships of variables ensure that it outperforms our other techniques in predicting short term electricity demand.
...
AEMO produces a short term forecast model in 30 minute intervals (the 'Half-hourly demand model') [@AEMOforecastwebsite], that uses features such as a categorical values for each half hour block of the day and month of the year, dummy variables for weekends and public holidays and transformations of half hourly temperature inclusive of interaction terms. This model uses a Least Absolute Shrinkage and Selection Operator (LASSO) regularisation algorithm which is a regression analysis method that automatically performs feature selection by including a penalty term to the loss function which acts to shrink the non significant variable coefficients to zero.

Our models seek to build upon the AEMO 'Half-hourly demand model'. At the most recent quarterly weighted average electricity spot price of \$148 per Megawatt hour (MWh) [@AER2023] for the region of NSW, the mean absolute error rate of the AEMO forecast roughly equated to AUD$463k of dollar value per hour, which highlights that there is a lot value to be gained in a more accurate forecasting methodology. To improve the modelling, we will explore if other meteorological data such as solar exposure can capture the impact of household solar photovoltaic generation in the grid, and whether rainfall has an impact on indoor electricity consumption. Our investigation seeks to compare the performance of various time series and machine learning models such as LASSO, Support Vector Regression (SVR) and Random Forest Regression with the aim of improving the short term electricity demand forecast with a slight increase in computational cost. The final model will be selected after comparison of various generated models and features, specifically through consideration of feature selection, accuracy, and interpretability and performance relative to AEMO's forecast.
...
Our paper seeks to build upon the success in the literature above by applying similar machine learning methods (with additional meteorological data) to generate a comparison of the techniques to identify the best performing model specific to the NSW grid. In @WangZhe2021Pcde, it is apparent that the best model approach and subset of features are heavily dependent on characteristics (including geographic and demographic) of that area's grid, so while these machine learning methods above have been heavily researched, we seek to understand which one of them can outperform the AEMO demand forecast specifically for the state of NSW grid.

# Material and Methods

## Software

Our analysis and modelling were conducted using the R programming language [@R], known for its proficiency in statistical analysis and data manipulation. We leveraged several essential R packages to streamline data management and analysis, empowering us to build robust predictive models. Some of the key R packages we used include: 

- **tidyverse:** This comprehensive package suite equipped us with a powerful set of tools for data manipulation, visualization, and analysis. Its flexibility and integration with other packages were essential for our data preparation and exploration [@tidyverse]. 

- **lubridate:** Essential for working with date and time data, a crucial aspect of time series analysis. This package allowed us to handle temporal information efficiently and accurately [@lubridate]. 

- **data.table:** We harnessed the efficiency of the data.table package for data manipulation tasks, particularly when dealing with large datasets. Its speed and concise syntax were instrumental in aggregating and selecting columns [@data.table]. 

- **crayon:** Used for adding colourful text to our visualizations, enhancing their interpretability, and making our reports more engaging [@crayon]. 

- **zoo:** As a pivotal package for handling time series data, zoo played a crucial role in our analysis. It provided the tools needed to process, manipulate, and visualize time-based data structures [@zoo]. 

- **tsibble:** Facilitating the management of time series data, tsibble served as the foundation for our time-based operations, ensuring consistency and accuracy in our analyses [@tsibble]. 

- **gridExtra, grid, patchwork:** These packages enabled us to create complex grid layouts for our visualizations, enhancing the presentation of our findings and insights [@gridExtra][@R][@patchwork]. 

- **GGally:** This package expanded our ggplot2-based visualizations by offering additional plot types and features, enriching our data exploration [@GGally]. 

## Data Acquisition

The data sets used in the analysis contained historical data from the state of NSW inclusive of the period 01-01-2010 to 01-08-2022 comprising historical electricity demand time series and meteorological observations over the period. Each data set has been stored in our public Github [@githubrepo]. The data includes the following data extracts;
...
- During Day = If the hour is between 8 am ~ 8 pm then 1, else 0.(this timeframe is selected due to having positive correlation to total energy demand, refer to Figure 13)
...
```{r eda_plot_1, fig.cap = "Line chart over time at hourly level", out.height = "60%", out.width = "90%"}
...
```{r eda_plot_2, fig.cap = "Line chart over time at monthly level", out.height = "35%", out.width = "90%"}
...
```{r eda_plot_4, fig.cap = "Monthly Total Energy Demand and AEMO Forecast", out.height = "35%", out.width = "90%"}
#Combine as grid
...
```{r eda_plot_5, fig.cap = "Comparing Monthly Residuals", out.height = "35%", out.width = "90%"}
...
```{r eda_plot_6, fig.cap = "Boxplot of Hourly Total demand by Year/Month/Hour", out.height = "50%", out.width = "90%"}
#Create Fig 6                                
...
```{r eda_plot_7, fig.cap = "Density of total demand by Season", out.height = "25%", out.width = "70%"}
#Draw Figure 7.1
grid.arrange(season_demand_density,
             ncol=1)
```

```{r eda_plot_8, fig.cap = "Density of total demand by Month", out.height = "25%", out.width = "70%"}
#Draw Figure 7.2
grid.arrange(month_demand_density,
             ncol=1)
```

```{r eda_plot_9, fig.cap = "Density of total demand by Extrene Temperature", out.height = "25%", out.width = "70%"}
#Draw Figure 7.3
...
Using the correlation matrix, it is possible to identify how strong are the relationships between each variable [@HaddAlexandria2021Ucm]. When the data is inspected at correlation perspective via figure 11, from total energy demand perspective, the highest correlated variables are nearly always associated with cold temperature. For example, it is understandable where temperature does not hold a strong correlation with total energy demand, given in figure 10, where the scatter plot does not exhibit a linear trend; however, from the correlation matrix of  figure 11, fields such as cold day, extreme cold day, and winter flag hold some of the highest correlation with total energy demand, indicating triggering those flags would result in higher total energy demand. Another field that holds a high correlation of 0.5 with total energy demand is during_day (flagging hours between 8 am ~ 8 pm), and this again strengthens assumption 2 of energy consumption being higher during human active hours. Furthermore, the negative correlation of -0.2 of datehour with total energy demand again contradicts assumption 3, hence we can settle with the statement that total energy demand is indeed gradually reducing over time. 

\newpage

```{r eda_plot_12, fig.cap = "Correlation Matrix of Total Energy Demand by Month", out.height = "35%", out.width = "90%"}
...
In the correlation matrix of total energy demand against month flags, via figure 12, the highest correlation can be identified in July, as well as August, indicating higher energy demand during these two months. This is quite surprising as August had much less increase in terms of total energy demand compared to June and July as per Figure 6. This indicates that perhaps both July and August should be kept for creating the predictive model.

```{r eda_plot_13, fig.cap = "Correlation Matrix of Total Energy Demand by Hour", out.height = "35%", out.width = "90%"}
...
Upon inspecting the correlation matrix of total energy demand against hour flags in Figure 13, it can be identified that hour 3 to 5 have the highest negative correlation, whilst hour 17 ~ 9 have the highest positive correlation. The correlation matrix of the hour flag against total energy demand again demonstrates that there is a pattern between total energy demand and hour of day.
...
Exploring more complex models as referenced in the literature review such as Generalised Additive Models [@McCullochJames2020IEDa], Long Short-Term Memory recurrent neural networks [@AlonsoAndresM.2020ASSL], and deep Feedforward Neural Networks [@9282124] could address some limitations experienced by the models chosen in this project, such as accounting for seasonality and being more robust against outliers. 
...
As renewable electricity generation becomes the focus of our future energy needs, quick start electricity generators such as pumped hydro will need to act as a stabiliser in the grid due to the intermittency of solar photovoltaic and wind generation. To ensure operational profitability of pumped hydro electricity generation, a more reliable short term demand forecast is needed. While AEMO's half hourly forecast has strong explanatory value, its predictive capability leaves room for improvement with a MAPE of 3.30%, equivalent to AUD$463k per hour at current prices.

Each of the models investigated were all able to outperform the AEMO forecast which we suspect was largely due to the inclusion of lagged demand variables taking inspiration from the original auto-regressive models used in earlier electricity load forecasting. In a sense, the inclusion of lagged demand is a trade-off between having an explanatory model, to improving predictive capability. Our LASSO and random forest regression models illustrate that 1-3 hour lagged demand is better at forecasting future demand likely as it already captures most of the other features (albeit 1 hour old) such as temperature, with the change of variables between periods not enough to add more significant information. This is beneficial as it helps to simplify the models without needing to add any exogenous variables.

Our Support Vector Regression model outperformed each of the other techniques explored, with a MAPE of 1.90%. This bettered the AEMO 'half hourly' forecast MAPE by 1.40%, which roughly equates to AUD$209k per forecast hour in economic value. We conclude that the use of a radial kernel was able to model the non linear patterns of the variables helping the Support Vector Regression model to extract the key predictive value from each feature increasing total accuracy.

Finally, we found the addition of solar exposure and rainfall had limited predictive power in each of the models. While we hoped that the addition of this data would help to incorporate the impact of solar photovoltaic generation and show an indoor/outdoor consumption trend in demand, it's limited impact might be explained by the insufficient granularity of the data. A more thorough approach maybe needed to fully discount these affects specifically for the NSW electricity grid.
...
Rmarkdown file link: https://github.com/llong-unsw/project-group-L/blob/main/Assessment%203%20-%20Group%20Project%20Report.Rmd

## 01 set up code

Code location: https://github.com/llong-unsw/project-group-L/blob/main/code/01%20Setup.R
...
## 02 Data exporation code

Code location: https://github.com/llong-unsw/project-group-L/blob/main/code/02%20Data%20exploration.R
...
Code location: https://github.com/llong-unsw/project-group-L/blob/main/code/03%20Modelling.R
